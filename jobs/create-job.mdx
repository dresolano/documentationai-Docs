...
<Steps>
  ...
  **Archived** is not available when you create a new job.
  ...
  If the linked client has an email address on file, a pre-visit confirmation email is automatically sent to them. The email includes the job title, date, and time, along with a button linking to the pre-visit confirmation form where the client can confirm or request a reschedule. NoSavvy sends this email once per job per day to avoid duplicates.
